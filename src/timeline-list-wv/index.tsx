...
import TimelineWV, {TimelineWVProps} from '../timeline-wv/TimelineWV';
import InfiniteListWV from '../infinite-list-wv';
...
export interface TimelineListWVProps {
  /**
   * Map of all timeline events ({[date]: events})
   */
  events: {[date: string]: TimelineWVProps['events']};
  /**
   * General timeline props to pass to each timeline item
   */
  timelineProps?: Omit<
    TimelineWVProps,
    'events' | 'scrollToFirst' | 'showNowIndicator' | 'scrollToNow' | 'initialTime'
  >;
  /**
   * Pass to render a custom Timeline item
   */
  renderItem?: (timelineProps: TimelineWVProps, info: TimelineListRenderItemInfo) => JSX.Element;
  /**
   ...
   */
  initialTime?: TimelineWVProps['initialTime'];
}

const TimelineList = (props: TimelineListWVProps) => {
