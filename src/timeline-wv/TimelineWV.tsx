...
export interface TimelineWVProps {
   ...
   */
  onBackgroundLongPress?: TimelineHoursWVProps['onBackgroundLongPress'];
  /**
   * Pass to handle creation of a new event by long press out on the timeline background
   * NOTE: If passed, the date prop will be included in the returned time string (e.g. 2017-09-06 01:30:00)
   */
  onBackgroundLongPressOut?: TimelineHoursWVProps['onBackgroundLongPressOut'];
...
const TimelineWV = (props: TimelineWVProps) => {
  const {
    format24h = true,
    start = 8,
    end = 19,
...
export default React.memo(TimelineWV);
